...
public class CompareDialog extends JDialog {
    ...
    private static String generateSource(BibEntry bibEntry, BibEntry compEntry) {
        ...
        if (bibEntry != null && compEntry != null) {
            ...
            //TODO: Take care of Optional checks like isPresenet() - zellerdev
            if (!bibEntry.getField(FieldName.TYPE).equals(compEntry.getField(FieldName.TYPE))
                ...
                source.append("<span style=\"background: yellow\">" + "@"
                        + bibEntry.getField(FieldName.TYPE).get().toUpperCase() + "{"
                        + bibEntry.getCiteKeyOptional().get() + "</span>");
            } else {
                source.append("@" + bibEntry.getField(FieldName.TYPE).get().toUpperCase() + "{"
                        + bibEntry.getCiteKeyOptional().get());
            }
            ...
            for (String field : commonFields) {
                ...
                if (FieldName.OWNER.equals(field))
                    continue;
                ...
                if (!field.startsWith("__") && !field.equals("id")
                        ...
                        && bibEntry.getField(field) != null
                        && !bibEntry.getField(field).get().isEmpty()) {

                    // compare values of src and comp entry
                    if (compEntry.getField(field) != null
                            && !compEntry.getField(field).get().isEmpty()
